...
if __name__ == "__main__":
    ...
    resume_data = load_and_validate_yaml(template_path)
    
    if resume_data:
        print(f"Contact name: {resume_data.contact.name}")
        print(f"Number of jobs: {len(resume_data.experience.jobs)}")
        print(f"Total skills: {len(resume_data.skills.list)}")
        
        # You can now use resume_data with full type safety!
        for job in resume_data.experience.jobs:
